...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awflow
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awflow-*.whl dist/*.tar.gz ...]
...
#: scans. __init__ is the entry point; dispatch is the scheduler; runtime is
#: the execution engine; journal is the persistence layer.
_KEYSTONES = ("awflow/__init__.py", "awflow/dispatch.py", "awflow/runtime.py", "awflow/journal.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awflow-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awflow/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awflow/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awflow/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awflow/dispatch.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awflow/y.py": "from awflow.models import Model\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awflow-0.0.0.tar.gz"
